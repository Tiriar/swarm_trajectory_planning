...
def start(clientID, quads, targets, speed, proxs, nodes, paths):
    """
    ...
    :param nodes: graph nodes positions
    :param paths: path indices for each quadrotor
    """
    ...
    kC = [0.20, 0.0]    # cohesion constants [multiplication const, power const]
    ...
    visible = [[0 for _ in range(quadsNum)] for _ in range(quadsNum)]   # visible quadrotors
    platoon = [[0 for _ in range(quadsNum)] for _ in range(quadsNum)]   # quads on the same path
    ...
    while vrep.simxGetConnectionId(clientID) != -1:
        ...
        for i in range(quadsNum):
            nrm = ut.norm(ut.sub(position[i][0:2], nodes[paths[0][-1]][0:2]))
            if nrm < 2.5 and not finished[i]:
                finished[i] = True
                print('Quad #'+str(i)+' finished in '+str((vrep.simxGetLastCmdTime(clientID)-t1)/1000)+'s')
                if all(_ for _ in finished):
        ...
        # compute quadrotors in platoon
        for i in range(quadsNum):
            for j in range(quadsNum):
                if i != j:
                    if visible[i][j]\
                            and paths[i][individualTarget[i]] in paths[j]\
                            and paths[j][individualTarget[j]] in paths[i]:
                        platoon[i][j] = 1
                    else:
                        platoon[i][j] = 0
        ...
        for i in range(quadsNum):
                ...
                if i != j and visible[i][j] == 1:
                    ...
                    if nrm != 0:
                        if platoon[i][j] == 1:
                            ...
                            temp = ut.mul(temp, kS[0]/(3*nrm**kS[1]))
            ...
            if sum(platoon[i]) != 0:
                for j in range(quadsNum):
                    if i != j and platoon[i][j] == 1:
                        temp = ut.mul(position[j], 1/sum(platoon[i]))
                        center = ut.add(center, temp)
                        temp = ut.mul(velocity[j], 1/sum(platoon[i]))
            ...
            nrm = ut.norm(ut.sub(position[i][0:2], nodes[paths[i][individualTarget[i]]][0:2]))
            if individualTarget[i] != 0:
                vec1 = ut.sub(nodes[paths[i][individualTarget[i]-1]], nodes[paths[i][individualTarget[i]]])
                vec2 = ut.sub(position[i], nodes[paths[i][individualTarget[i]]])
                if individualTarget[i] < len(paths[i])-1 and (nrm <= 1 or ut.angle(vec1, vec2) >= math.pi/2):
            ...
            destination[i] = ut.sub(nodes[paths[i][individualTarget[i]]], position[i])
